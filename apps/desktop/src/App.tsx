...
import { AgentStudio, AiAssistantPanel } from "./features/agent-studio";
import { HelpCenter } from "./features/help";
import { JobProgress, EditableBlueprintOptionField, formatBlueprintOption, AuroraSyncPanel } from "./features/shared";
import { DialogueSummaryView, DialogueWorkspace, EditableDialogueField, EditableLocalizedDialogueField, Metric } from "./features/dialogues";
import {
...
const MapCreator = lazy(() => import("./features/map-creator/MapCreator").then((module) => ({ default: module.MapCreator })));
const AreaMigrationWorkspace = lazy(() => import("./features/exports/AreaMigrationWorkspace").then((module) => ({ default: module.AreaMigrationWorkspace })));
const AssetExportWorkspace = lazy(() => import("./features/exports/AssetExportWorkspace").then((module) => ({ default: module.AssetExportWorkspace })));
const DialogueExportWorkspace = lazy(() => import("./features/exports/DialogueExportWorkspace").then((module) => ({ default: module.DialogueExportWorkspace })));
...
const explorerGroupDefinitions = [
  ...
  { id: "migration", label: "Exporter une carte", icon: Download, section: "Construire" },
  { id: "asset_export", label: "Exporter des assets", icon: SquareStack, section: "Exporter" },
  { id: "dialogue_export", label: "Exporter des dialogues", icon: MessageSquareText, section: "Exporter" },
...
function App() {
  const activeExplorerItem = useWorkbenchStore((state) => state.activeExplorerItem);
  const setActiveExplorerItem = useWorkbenchStore((state) => state.setActiveExplorerItem);
  const lastContentView = useWorkbenchStore((state) => state.lastContentView);
  const setLastContentView = useWorkbenchStore((state) => state.setLastContentView);
  const agentObjectiveDraft = useWorkbenchStore((state) => state.agentObjectiveDraft);
  const setAgentObjectiveDraft = useWorkbenchStore((state) => state.setAgentObjectiveDraft);
  const explorerOpen = useUiStore((state) => state.explorerOpen);
  const toggleExplorerOpen = useUiStore((state) => state.toggleExplorerOpen);
  const inspectorOpen = useUiStore((state) => state.inspectorOpen);
  const setInspectorOpen = useUiStore((state) => state.setInspectorOpen);
  const toggleInspectorOpen = useUiStore((state) => state.toggleInspectorOpen);
  const diagnosticsOpen = useUiStore((state) => state.diagnosticsOpen);
  const toggleDiagnosticsOpen = useUiStore((state) => state.toggleDiagnosticsOpen);
  ...
  const standaloneOpenedJob = useRef<string | undefined>(undefined);
  ...
  const standaloneArea = /\.are$/i.test(project.modulePath);
  ...
  const explorerGroups = useMemo(
    () =>
      explorerGroupDefinitions.map((item) => ({
        ...item,
        count:
          ["build", "map_creator", "migration", "asset_export", "dialogue_export", "agent", "help"].includes(item.id)
            ? undefined
            : item.id === "dialogues"
  ...
  useEffect(() => {
    if (
      standaloneArea &&
      job?.state === "completed" &&
      job.id !== standaloneOpenedJob.current
    ) {
      standaloneOpenedJob.current = job.id;
      setActiveExplorerItem("areas");
    }
  }, [job?.id, job?.state, setActiveExplorerItem, standaloneArea]);
  ...
  useEffect(() => {
    if (["dialogues", "areas", "journal", "factions", "blueprints", "map_creator", "migration", "asset_export", "dialogue_export", "agent"].includes(activeExplorerItem)) {
  ...
  async function openEditWorkspace() {
    ...
    if (standaloneArea) {
      setDiagnostics((current) => [
        ...current.filter((item) => item.id !== "STANDALONE_AREA_READ_ONLY"),
        {
          id: "STANDALONE_AREA_READ_ONLY",
          level: "info",
          code: "STANDALONE_AREA_READ_ONLY",
          message: "Une carte .are ouverte seule reste en lecture seule. Ouvrez un module .mod pour créer un espace d’édition.",
        },
      ]);
      return;
    }
    ...
    <main className={`app-shell rpg-shell ${diagnosticsOpen ? "diagnostics-open" : ""}`}>
      ...
      <section className={`workspace-grid ${explorerOpen ? "" : "explorer-collapsed"} ${inspectorOpen ? "" : "inspector-collapsed"}`}>
        <aside className="explorer panel" aria-label="Explorateur du module">
          <div className="panel-title">
            ...
            <button type="button" className="icon-button" aria-label={explorerOpen ? "Réduire l'explorateur" : "Afficher l'explorateur"} onClick={toggleExplorerOpen}>
        ...
        <section className="workbench panel" aria-label="Zone de travail">
          ...
          <div className="forge-canvas">
            ...
            <div className="welcome-heading">
              ...
              {moduleInfo ? (
                <p>
                  {standaloneArea ? "Carte autonome" : "Module"} <code>{moduleInfo.tag}</code>
                  {!standaloneArea && <> · NWN {moduleInfo.minimumGameVersion ?? "non spécifiée"} · zone d'entrée <code>{moduleInfo.entryArea}</code></>}
                </p>
              ) : (
                <p>
                  Sélectionnez un module complet ou une carte .are autonome. Les racines NWN permettent
                  de résoudre les tilesets et modèles, sans modifier aucune ressource.
                </p>
              )}
            </div>
            ...
            <div className="project-form">
              <PathField
                label="Module ou carte NWN"
                hint="Fichier .mod complet ou carte .are autonome en lecture seule"
            ...
            {!jobId && sessionRestoreState !== "restoring" && (
              ...
              <aside className="campaign-overview scroll-panel">
                ...
                {job?.state === "completed" && !editWorkspace && !standaloneArea && (
                ...
                {job?.state === "completed" && standaloneArea && (
                  <div className="safety-note">
                    <ShieldCheck size={18} />
                    <div><strong>Carte autonome en lecture seule</strong><p>Les fichiers voisins .git/.gic et les ressources du jeu sont résolus sans créer d’espace d’édition.</p></div>
                  </div>
                )}
                {editWorkspace && (
            ...
            {activeExplorerItem === "migration" && (
              <Suspense fallback={<div className="locked-workspace">Chargement de l’exportateur de cartes…</div>}>
                <AreaMigrationWorkspace jobId={jobId} analysisReady={job?.state === "completed"} />
              </Suspense>
            )}

            {activeExplorerItem === "asset_export" && (
              <Suspense fallback={<div className="locked-workspace">Chargement de l’exportateur d’assets…</div>}>
                <AssetExportWorkspace jobId={jobId} analysisReady={job?.state === "completed"} />
              </Suspense>
            )}

            {activeExplorerItem === "dialogue_export" && (
              <Suspense fallback={<div className="locked-workspace">Chargement de l’exportateur de dialogues…</div>}>
                <DialogueExportWorkspace jobId={jobId} analysisReady={job?.state === "completed"} workspaceId={editWorkspace?.workspaceId} />
              </Suspense>
            )}
        ...
        <aside className="inspector panel" aria-label="Inspecteur">
          <div className="panel-title">
            ...
            <button type="button" className="icon-button" aria-label={inspectorOpen ? "Réduire l'inspecteur" : "Afficher l'inspecteur"} onClick={toggleInspectorOpen}>
      ...
      <section className={`diagnostics panel ${diagnosticsOpen ? "expanded" : "collapsed"}`} aria-label="Diagnostics">
        <div className="diagnostic-tabs">
          <button type="button" className="active" aria-expanded={diagnosticsOpen} onClick={toggleDiagnosticsOpen}>
...
export function sceneGroundPosition(
  width: number,
  height: number,
  objects: ReadonlyArray<{ kind: string; y: number }>,
) {
  return { x: width * 5, y: sceneGroundElevation(objects), z: height * 5 };
}
...
function BabylonScene({ jobId, manifest, showOverlays, showWalkmeshes, showTileFade, wireframe, cameraMode }: { jobId: string; manifest: SceneManifest; showOverlays: boolean; showWalkmeshes: boolean; showTileFade: boolean; wireframe: boolean; cameraMode: "orbit" | "aurora" }) {
  ...
  useEffect(() => {
    ...
    void import("@babylonjs/core").then(async (B) => {
      ...
      const ground = B.MeshBuilder.CreateGround("ground", { width: Math.max(10, manifest.width * 10), height: Math.max(10, manifest.height * 10) }, scene); const groundPosition = sceneGroundPosition(manifest.width, manifest.height, manifest.objects); ground.position = new B.Vector3(groundPosition.x, groundPosition.y, groundPosition.z); const groundMaterial = new B.StandardMaterial("ground-material", scene); groundMaterial.diffuseColor = new B.Color3(0.12, 0.18, 0.22); ground.material = groundMaterial;
      ...
      const groupedModels = [...groups.entries()];
      const preparationBatchSize = 64;
      for (let batchStart = 0; batchStart < groupedModels.length; batchStart += preparationBatchSize) {
        if (disposed) break;
        const batch = groupedModels.slice(batchStart, batchStart + preparationBatchSize);
        setStatus(`Préparation parallèle des modèles · ${batchStart}/${groupedModels.length}`);
        await prepareSceneModels({ jobId, resrefs: batch.map(([resref]) => resref) }).catch(() => null);
      }
      if (disposed) return;
      setStatus(`Préparation parallèle des modèles · ${groupedModels.length}/${groupedModels.length}`);
      for (const [resref, values] of groupedModels) {
          if (disposed) break;
          try {
            ...
            for (const value of values) {
              const instance = container.instantiateModelsToScene((name) => value.id + ":" + name, false); const anchor = new B.TransformNode("anchor:" + value.id, scene); anchor.position = new B.Vector3(value.x, value.y, value.z); anchor.rotation.y = value.rotation; anchor.metadata = value;
