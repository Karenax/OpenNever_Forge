...
const topics: GuideTopic[] = [
  {
    ...
    summary: "Identifier le MOD ou la carte ARE, l’installation du jeu et le dossier utilisateur sans confondre leurs rôles.",
    icon: Settings2,
    prerequisites: ["Neverwinter Nights: Enhanced Edition est installé.", "Vous disposez d’une copie du fichier .mod ou d’une carte .are à étudier."],
    steps: [
      { title: "Choisir la source", instruction: "Sélectionnez un module .mod complet ou une carte .are autonome. Pour un ARE, OpenNever charge aussi les fichiers .git/.gic et les ressources reconnues placés dans le même dossier, toujours en lecture seule.", control: "Parcourir — Module ou carte NWN", expected: "Le chemin absolu du .mod ou du .are apparaît dans le premier champ." },
    ...
    troubleshooting: [
      { problem: "Le bouton d’analyse reste désactivé.", resolution: "Renseignez au minimum un fichier .mod ou .are valide. Les deux racines NWN restent recommandées pour résoudre tous les assets." },
  ...
  },
  {
    ...
    icon: Compass,
    prerequisites: ["La source NWN est renseignée.", "Le fichier .mod ou .are sélectionné est une copie de travail."],
    steps: [
    ...
    troubleshooting: [
      { problem: "L’analyse échoue immédiatement.", resolution: "Vérifiez que le fichier existe, porte l’extension .mod ou .are et n’est pas verrouillé par une copie ou un téléchargement incomplet." },
      { problem: "Une carte ARE n’affiche pas ses objets.", resolution: "Placez son fichier .git portant le même ResRef dans le même dossier. Le .gic voisin est également chargé lorsqu’il existe." },
      { problem: "L’édition n’est pas proposée pour une carte ARE.", resolution: "Le mode ARE autonome est volontairement en lecture seule. Ouvrez un module .mod complet pour créer un espace d’édition et construire un nouveau MOD." },
  ...
  },
  {
    id: "migration",
    category: "Exporter",
    title: "Préparer un bundle de migration de zone",
    summary: "Auditer une zone analysée puis produire un bundle local neutre sans modifier les sources NWN.",
    icon: Map,
    prerequisites: ["Le module et ses dépendances ont été analysés.", "Vous avez choisi une zone et un dossier de destination local distinct des sources."],
    steps: [
      { title: "Choisir la zone analysée", instruction: "Ouvrez Exporter une carte puis sélectionnez une zone dans la colonne gauche. Le moteur utilise le catalogue et l’index du monde déjà produits par l’analyse active.", expected: "Le panneau central affiche les tuiles, instances, modèles, textures, navigations et éléments manquants prévus." },
      { title: "Lire l’audit", instruction: "Contrôlez les diagnostics et les statuts exact, converti, placeholder, non pris en charge ou manquant. Une erreur bloque le lancement ; un avertissement reste présent dans diagnostics.jsonl.", expected: "Aucun élément inconnu ou absent n’est omis silencieusement." },
      { title: "Choisir la destination", instruction: "Sélectionnez un dossier parent accessible. OpenNever propose un nouveau sous-dossier .area-migration-v1 et refuse d’écraser une destination existante.", control: "Choisir la destination", expected: "Le chemin complet du futur bundle apparaît sans viser le MOD, un HAK, un TLK ou l’installation." },
      { title: "Accepter la limite de redistribution", instruction: "Lisez puis cochez l’avertissement local-only. Les ressources propriétaires converties ou préservées ne deviennent pas redistribuables par leur présence dans le bundle.", expected: "Le bouton Exporter le bundle devient disponible seulement après cette confirmation." },
      { title: "Exporter et contrôler", instruction: "Lancez le job, suivez sa progression ou annulez-le. À la fin, ouvrez le chemin indiqué et contrôlez migration-report.json et diagnostics.jsonl.", control: "Exporter le bundle", expected: "Le dossier apparaît atomiquement avec manifest.json, area.json, identity-map.json, diagnostics, rapport et assets résolus." },
    ],
    outcome: "Vous obtenez un Area Migration Bundle v1 déterministe, inventorié par taille/SHA-256, avec navigation source préservée mais explicitement non convertie.",
    warning: "Le bundle reste local et peut contenir des ressources propriétaires. Ne le publiez pas sans disposer séparément de tous les droits nécessaires.",
    troubleshooting: [
      { problem: "La zone n’apparaît pas.", resolution: "Relancez l’analyse du bon MOD avec ses racines installation et données utilisateur ; la CLI et le desktop n’inventent pas de catalogue manquant." },
      { problem: "L’export est bloqué.", resolution: "Corrigez les diagnostics de gravité erreur, choisissez une destination qui n’existe pas encore et acceptez l’avertissement local-only." },
      { problem: "Le rapport mentionne des placeholders.", resolution: "Cherchez le code correspondant dans diagnostics.jsonl puis vérifiez la version sélectionnée et les versions masquées dans manifest.json." },
    ],
    action: { label: "Ouvrir Exporter une carte", view: "migration" },
  },
  {
    id: "asset-export",
    category: "Exporter",
    title: "Exporter un asset statique ou animé",
    summary: "Convertir un modèle Aurora en GLB avec ses textures et ses clips d’animation résolus.",
    icon: Boxes,
    prerequisites: ["Une carte ou un module a été analysé.", "Le modèle MDL est résolu par le module, ses HAK ou l’installation NWN.", "La destination est distincte des sources NWN."],
    steps: [
      { title: "Choisir le modèle", instruction: "Ouvrez Export puis Exporter des assets. Recherchez le ResRef et utilisez les filtres Animés ou Statiques pour réduire la liste.", expected: "La fiche indique les mesh, skins, textures et animations réellement transformables." },
      { title: "Contrôler les animations", instruction: "Pour un modèle animé, vérifiez chaque clip, sa durée, ses pistes et ses événements. Un clip sans piste transformable est signalé et n’est pas présenté comme exporté.", expected: "Le nombre de clips GLB correspond aux animations marquées Exportée." },
      { title: "Contrôler les textures", instruction: "Vérifiez les PNG prévus et les replis de matériau. Les textures DDS, TGA, PNG ou PLT prises en charge sont converties localement avec des limites mémoire strictes.", expected: "Chaque texture résolue possède une destination ; toute texture manquante reste explicitement signalée." },
      { title: "Choisir la destination", instruction: "Sélectionnez un dossier parent. OpenNever propose un nouveau dossier <resref>.asset-export-v1 et refuse d’écraser un export existant ou d’écrire sous une racine NWN protégée.", control: "Choisir la destination", expected: "Le chemin proposé ne vise ni le MOD, ni un HAK, ni l’installation ou les données utilisateur." },
      { title: "Exporter", instruction: "Confirmez que l’export reste local puis lancez la conversion. Le dossier final est publié atomiquement après une seconde vérification des empreintes des modèles sources.", control: "Exporter l’asset", expected: "Le dossier contient le GLB, manifest.json et le sous-dossier textures si nécessaire." },
    ],
    outcome: "Vous obtenez un dossier d’asset inventorié par SHA-256, statique ou animé selon les clips réellement écrits dans le GLB.",
    warning: "La conversion ne modifie pas les droits des ressources NWN. Conservez l’export local sauf si vous détenez séparément les droits de redistribution.",
    troubleshooting: [
      { problem: "Le modèle apparaît statique malgré des noms d’animation.", resolution: "Les animations peuvent être déclarées sans piste transformable ou cibler des nœuds absents. Consultez le détail des clips et les avertissements de l’aperçu." },
      { problem: "Une texture utilise le repli matériau.", resolution: "Vérifiez que le ResRef est résolu dans un format DDS, TGA, PNG ou PLT pris en charge et que la source n’a pas changé depuis l’analyse." },
      { problem: "La destination est refusée.", resolution: "Choisissez un dossier existant hors des répertoires NWN, puis laissez OpenNever créer un nouveau sous-dossier qui n’existe pas encore." },
    ],
    action: { label: "Ouvrir Exporter des assets", view: "asset_export" },
  },
  {
    id: "dialogue-export",
    category: "Exporter",
    title: "Exporter un dialogue",
    summary: "Préserver un DLG et produire une structure JSON ainsi qu’un transcript lisible et vérifiable.",
    icon: MessageSquareText,
    prerequisites: ["Une carte ou un module a été analysé.", "Le dialogue existe dans l’analyse ou dans le workspace d’édition.", "La destination est distincte des sources NWN."],
    steps: [
      { title: "Choisir le dialogue", instruction: "Ouvrez Export puis Exporter des dialogues. Recherchez le ResRef ou un extrait de texte dans la colonne gauche.", expected: "La fiche affiche les lignes, liens, racines, cycles, scripts et références du dialogue sélectionné." },
      { title: "Contrôler la version", instruction: "Vérifiez le badge Version analysée ou Version modifiée du workspace. Si un dialogue a été édité ou créé dans l’overlay, cette version est prioritaire.", expected: "Le SHA-256 et l’aperçu correspondent aux octets DLG qui seront exportés." },
      { title: "Lire la structure", instruction: "Examinez le transcript et les avertissements. Les cycles, liens cassés, nœuds inaccessibles et diagnostics sont conservés plutôt que supprimés.", expected: "Les anomalies connues restent visibles avant l’écriture du bundle." },
      { title: "Choisir la destination", instruction: "Sélectionnez un dossier parent. OpenNever propose un nouveau sous-dossier .dialogue-export-v1 et refuse les destinations existantes ou protégées.", control: "Choisir la destination", expected: "Le chemin complet apparaît sans viser un MOD, un HAK, un TLK ou l’installation." },
      { title: "Exporter et vérifier", instruction: "Acceptez l’avertissement local-only puis lancez l’export. Contrôlez le DLG, dialogue.json, transcript.md et manifest.json.", control: "Exporter le dialogue", expected: "Le dossier apparaît atomiquement avec quatre fichiers et leurs empreintes d’intégrité." },
    ],
    outcome: "Vous obtenez le DLG exact sélectionné, un JSON portable sans chemins locaux, un transcript Markdown et un manifeste SHA-256.",
    warning: "Les textes, scripts et ressources NWN restent soumis à leurs droits d’origine. Ne publiez pas le bundle sans autorisation séparée.",
    troubleshooting: [
      { problem: "Le dialogue n’apparaît pas.", resolution: "Vérifiez que l’analyse est terminée. Pour un dialogue nouvellement créé, gardez le workspace d’édition ouvert afin que sa ressource DLG staged soit listée." },
      { problem: "Le transcript contient un texte non résolu.", resolution: "Le DLG peut référencer un StrRef absent du TLK résolu. Renseignez les racines de dépendances correctes puis relancez l’analyse." },
      { problem: "L’export est refusé.", resolution: "Choisissez une destination neuve hors des sources NWN et confirmez que l’export reste local." },
    ],
    action: { label: "Ouvrir Exporter des dialogues", view: "dialogue_export" },
  },
  {
    id: "diagnostics",
    category: "Dépanner",
