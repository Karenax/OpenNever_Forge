...
import { domains } from "./UxEnhancements.model";
...
describe("UX navigation helpers", () => {
  it("maps every principal workshop to its functional domain", () => {
    ...
    expect(domainForItem("migration")).toBe("export");
    expect(domainForItem("asset_export")).toBe("export");
    expect(domainForItem("dialogue_export")).toBe("export");
  ...
  it("places Export sixth and opens with the map exporter first", () => {
    expect(domains[5]?.id).toBe("export");
    expect(domains[5]?.itemIds[0]).toBe("migration");
    expect(domains[5]?.itemIds[1]).toBe("asset_export");
    expect(domains[5]?.itemIds[2]).toBe("dialogue_export");
    expect(domains[6]?.id).toBe("agent");
  });
