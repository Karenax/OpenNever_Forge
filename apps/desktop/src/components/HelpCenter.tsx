...
const topics: GuideTopic[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Placer une instance", instruction: "Choisissez la catégorie, saisissez le ResRef exact du blueprint, un tag puis X, Y, Z et l’orientation. X/Y placent l’objet au sol, Z règle l’altitude.", control: "+ Placer une instance → Ajouter à la zone", expected: "L’instance apparaît dans la liste et sur la vue de zone." },
  ...
  },
  {
    id: "narrative",
    category: "Éditer",
    title: "Écrire une quête cohérente",
    summary: "Relier dialogue, journal, conditions, actions et factions sans créer de branche morte.",
    icon: ScrollText,
    prerequisites: ["L’atelier transactionnel est créé.", "Les personnages, scripts et entrées de journal visés sont identifiés."],
    steps: [
      { title: "Dessiner le parcours", instruction: "Définissez d’abord l’état initial, les choix du joueur, les conditions d’accès et les états finaux. Donnez un objectif observable à chaque branche.", expected: "Chaque choix mène à une conséquence et chaque fin met à jour l’état de quête." },
      { title: "Construire le dialogue", instruction: "Créez les nœuds PNJ/joueur, reliez les réponses et nommez les scripts de condition et d’action avec des ResRef explicites.", expected: "Le graphe ne contient ni nœud inaccessible ni lien orphelin." },
      { title: "Relier le journal", instruction: "Ajoutez les entrées correspondant aux étapes de la quête et identifiez clairement les états terminés ou échoués.", expected: "Le journal reflète chaque transition importante du dialogue ou du gameplay." },
      { title: "Valider l’ensemble", instruction: "Contrôlez les références de scripts, les factions des interlocuteurs et les diagnostics de graphe avant de construire.", expected: "Tous les ResRef référencés existent et les branches prévues sont atteignables." },
    ],
    outcome: "La quête possède un chemin compréhensible, des états persistants et des références valides.",
    troubleshooting: [
      { problem: "Une réponse n’apparaît jamais en jeu.", resolution: "Vérifiez le script de condition, le sens du lien et les variables locales attendues au moment du dialogue." },
  ...
  },
  {
    ...
    steps: [
      { title: "Choisir le bon mode", instruction: "Utilisez l’Agent multi-étapes pour un objectif complexe et l’Assistant contrôlé pour obtenir une proposition JSON ponctuelle. Les deux suivent la politique de capacités.", expected: "Le mode choisi correspond à l’ampleur de la tâche, pas seulement au fournisseur utilisé." },
      { title: "Configurer le fournisseur", instruction: "Choisissez un protocole explicitement marqué OpenAI compatible ou Ollama, renseignez endpoint, modèle et clé temporaire. La clé n’est jamais persistée.", expected: "Les champs requis sont complets et l’accès réseau est autorisé dans le profil." },
      { title: "Tester la liaison", instruction: "Effectuez un appel minimal avant de lancer un objectif long. Le test vérifie l’endpoint, l’authentification, le modèle et le format de réponse.", control: "Tester la communication avec le modèle", expected: "Un état de réussite affiche le fournisseur, le modèle et le temps de réponse." },
      { title: "Préparer l’exécution", instruction: "Décrivez un résultat vérifiable, par exemple le nom de la zone, les objets attendus et les contraintes. Préparer crée le plan et le journal mais ne lance pas encore le modèle.", control: "Préparer l’exécution", expected: "Une carte d’exécution persistée apparaît avec l’état planned." },
    ...
    troubleshooting: [
      ...
      { problem: "Préparer fonctionne mais rien ne démarre.", resolution: "C’est normal : ouvrez l’exécution créée et cliquez sur Lancer l’agent. Préparer ne contacte pas le modèle." },
